# COGUARDIAN_PATCH__WATCHDOG_TASK_HIDDEN__V1
...
if($pwsh){
  $exe = $pwsh
  $args = "-WindowStyle Hidden -NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$script`""
} else {
...
$settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -StartWhenAvailable -MultipleInstances IgnoreNew -ExecutionTimeLimit (New-TimeSpan -Hours 1) -Hidden
